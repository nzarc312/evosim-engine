...
const char* TelemetryWriter::header() {
    return "tick,population,food_active,mean_speed,std_speed,mean_size,std_size,"
           "mean_sense,std_sense,mean_greed,std_greed,births,deaths,collapsed,"
           "recolonised,stock_total,harvest,mean_energy,ms_per_tick,state_hash";
}
...
void TelemetryWriter::flush() {
    ...
    for (const TelemetryRow& r : buf_) {
        ...
        const int n = std::snprintf(
            line, sizeof(line),
            "%llu,%llu,%llu,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,"
            "%llu,%llu,%llu,%llu,%.17g,%.17g,%.17g,%.6f,%llu\n",
            ...
            r.mean_greed, r.std_greed,
            static_cast<unsigned long long>(r.births),
            static_cast<unsigned long long>(r.deaths),
            static_cast<unsigned long long>(r.collapsed),
            static_cast<unsigned long long>(r.recolonised),
            r.stock_total, r.harvest,
